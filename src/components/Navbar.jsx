  ...
  return (
    <nav
      className={cn(
        ...
        isScrolled ? "py-4 bg-background/80 backdrop-blur-lg border-border shadow-sm" : "py-6 bg-transparent"
      )}
    >
        ...
        <div className="hidden md:flex space-x-8">
          {navItems.map((item, key) => (
            <a
              key={key}
              href={item.href}
              className="text-md font-semibold text-foreground/80 hover:text-primary transition-colors duration-300"
            >
              {item.name}
            </a>
          ))}
          ...
          <div className="flex flex-col space-y-8 text-xl text-center">
            {navItems.map((item, key) => (
              <a
                key={key}
                href={item.href}
                className="font-semibold text-foreground/90 hover:text-primary transition-colors duration-300"
